...
def solution():
    ...
    while q:
        ...
        if y > 0 and direction != "down" and (direction != "up" or dist < 10) and (direction not in ("right","left") or dist >= 4):
            heapq.heappush(q, (weight+grid[y-1][x], x, y-1, "up", dist+1 if direction == "up" else 1))
        if y < h-1 and direction != "up" and (direction != "down" or dist < 10) and (direction not in ("right","left") or dist >= 4):
            heapq.heappush(q, (weight+grid[y+1][x], x, y+1, "down", dist+1 if direction == "down" else 1))
        if x > 0 and direction != "right" and (direction != "left" or dist < 10) and (direction not in ("up","down") or dist >= 4):
            heapq.heappush(q, (weight+grid[y][x-1], x-1, y, "left", dist+1 if direction == "left" else 1))
        if x < w-1 and direction != "left" and (direction != "right" or dist < 10) and (direction not in ("up","down") or dist >= 4):
